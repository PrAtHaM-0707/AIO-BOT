...
const { serverConfigCollection, stickyMessageCollection, autoResponderCollection, userPreferencesCollection } = require('../mongodb');
...
module.exports = {
    ...
    async execute(message, client) {
        ...
        const userPreference = await userPreferencesCollection.findOne({ userId: message.author.id });
        const isOptedOut = userPreference?.optedOut || false;

        // Handle AFK (this doesn't track message content, so it runs for everyone)
        try {
        ...
        // Only run tracking features if the user hasn't opted out
        if (!isOptedOut) {
            // Counting Game
            ...
            // Custom Commands (skip if user is opted out, since optout/optin are slash commands only)
            try {
                let config;
                try {
        ...
        // Sticky Messages (this doesn't track message content, so it runs for everyone)
        try {
